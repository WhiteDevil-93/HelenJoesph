...
const TAB_NAMES={favourites:'Favs',all:'All','1_resuscitation_fluids_and_inotropes':'Resus','2_airway_and_ventilation':'Airway','3_sedation_analgesia_and_neurology':'Sedation','4_antimicrobials_and_infectious_diseases':'Antibiotics','5_metabolic_electrolytes_and_nutrition':'Metabolic','6_poisoning_and_toxicology':'Toxicology','7_useful_formulae':'Formulae','8_cardiovascular':'Cardiac','9_blood_products':'Blood','10_endocrine_and_other':'Endocrine','11_ed_medical_emergencies':'ED Medical','12_ed_toxicology':'ED Toxic','13_ed_trauma_surgical':'ED Trauma','14_ed_metabolic':'ED Metabolic','15_ed_procedures':'ED Proc','16_score_calculators':'Scores'};
...
const INFUSION_DRUGS=['noradrenaline','adrenaline','dopamine','dobutamine','phenylephrine','nitroglycerin','nitroprusside','lignocaine','amiodarone','furosemide','morphine','midazolam','propofol','atracurium','alteplase','heparin','insulin'];
...
function sW(v){
  ...
  if(W>0){
    showWeightToast(W);
    if(act==='all')renderAll();
    else if(act!=='favourites')renderCat(act);
  }
}
...
function showWeightToast(w){
  ...
  if(!t){
    ...
    t.id='wtoast';t.className='wtoast';
    document.body.appendChild(t);
  }
  t.textContent='Weight set: '+w+' kg';
  t.style.display='block';
  clearTimeout(t._tt);
  t._tt=setTimeout(()=>{t.style.display='none'},2000);
}
...
function mkNav(){
  ...
  for(const k of ORDER){
    if(k==='favourites'){const c=gF().length;h+=`<button class="cp${k===act?' on':''}" data-c="${k}" onclick="setCat('${k}')">${I[k]} ${TAB_NAMES[k]}${c?`<span class=n id=favCt>${c}</span>`:''}</button>`}
    else if(k==='all')h+=`<button class="cp on" data-c="all" onclick="setCat('all')">${I.all} All</button>`
    else if(D&&D[k])h+=`<button class="cp" data-c="${k}" onclick="setCat('${k}')">${I[k]} ${TAB_NAMES[k]}</button>`
  }
...
function renderEDProcedures(d){
  ...
  for(const it of protocols){
    ...
    if(parent.startsWith('Step By Step')){
      ...
      if(l==='action')stepMap[parent].action=it.notes_updates||'';
      else if(l==='details')stepMap[parent].details=it.notes_updates||'';
      else if(l==='caution')stepMap[parent].caution=it.notes_updates||'';
    }
  }
  ...
  if(!h)return'';
  return `<div class="sect open" data-c="15_ed_procedures"><div class="sh" onclick="togS(this)"><div style="display:flex;align-items:center"><div class="shi">${I['15_ed_procedures']}</div><div class="sht">${C['15_ed_procedures']}</div></div><div class="shc">▼</div></div><div class="sb">${h}</div></div>`;
}
...
function renderEDProtocolCard(proto,equip,steps){
  ...
  if(steps.length>0){
    ...
    for(let i=0;i<steps.length;i++){
      const s=steps[i];
      h+=`<div class="edstep">`;
      h+=`<div class="edstep-n">STEP ${i+1}</div>`;
      if(s.action)h+=`<div class="edstep-a">${s.action}</div>`;
      if(s.details)h+=`<div class="edstep-d">${s.details}</div>`;
      if(s.caution)h+=`<div class="edstep-c">⚠️ ${s.caution}</div>`;
      h+=`</div>`;
    }
  }
...
function parseProtocolSections(text){
  ...
  if(stepMatch){
    ...
    for(let i=0;i<stepTexts.length;i++){const st=stepTexts[i].trim();if(st.length>2)steps.push(st)}
  }
  if(steps.length===0){
  ...
  const features={};
  const sysPatterns={
    'Muscarinic':/Muscarinic[:\s]*([^|]*)/i,'Nicotinic':/Nicotinic[:\s]*([^|]*)/i,'CNS':/CNS[:\s]*([^|]*)/i,
    'Symptoms':/Symptoms[:\s]*([^|]*)/i,'Signs':/Signs[:\s]*([^|]*)/i,'Clinical Features':/Clinical Features[:\s]*([^|]*)/i
  };
  ...
  const decon={};
  const deconPatterns={
    'Activated Charcoal':/Activated Charcoal[:\s]*([^|]*)/i,'Gastric Lavage':/Gastric Lavage[:\s]*([^|]*)/i,
    'Skin Decon':/Skin Decon(?:tamination)?[:\s]*([^|]*)/i,'Whole Bowel':/Whole Bowel[:\s]*([^|]*)/i
  };
  for(const[item,pattern]of Object.entries(deconPatterns)){const m=text.match(pattern);if(m)decon[item]=m[1].trim()}
  const warnings=[];
...
function calcWeightDose(doseStr,wt){
  ...
  if(m2){
    ...
    const unitMap={ug:'mcg',mcg:'mcg',mg:'mg',g:'g',units:'units',meq:'mEq',mmol:'mmol',ml:'mL'};
    return v.toFixed(1).replace(/\.0$/,'')+' '+unitMap[unit];
  }
  return null;
}
...
function isInfusionDrug(name,it){
  ...
  for(const d of INFUSION_DRUGS)if(n.includes(d))return true;
  const dose=(it.adult_dose||it.adult_settings||it.paediatric_dose||it.protocol_dose||'').toString().toLowerCase();
  return dose.includes('/min')||dose.includes('infusion')||dose.includes('ivi')||dose.includes('mcg/kg/min')||dose.includes('mg/kg/hr')||dose.includes('units/hr');
}

/* ===== INFUSION CALCULATOR ===== */
function renderInfusionCalc(it,n){
  const kid='if-'+n.replace(/\s+/g,'_').replace(/[()]/g,'');
  return `<div class="ifbox" id="${kid}">
    <div class="ifh">💉 Infusion Calculator</div>
    <div class="ifrow"><label>Dose</label><input type="number" id="${kid}-d" placeholder="0.1" step="any"><span class="iu">mcg/kg/min</span></div>
    <div class="ifrow"><label>Conc</label><input type="number" id="${kid}-c" placeholder="4" step="any"><span class="iu">mg/mL</span></div>
    <div class="ifrow"><label>Weight</label><input type="number" id="${kid}-w" value="${W||''}" placeholder="kg" step="any"><span class="iu">kg</span></div>
    <button class="ifbtn" onclick="calcInfusion('${kid}')">Calculate mL/hr</button>
    <div class="ifres" id="${kid}-r" style="display:none"></div>
  </div>`;
}
...
function renderDrug(it,cat,n){
  ...
  if(W>0){
    ...
    if(ad.toString().toLowerCase().includes('/kg')||ad.toString().toLowerCase().includes('per kg')||ad.toString().toLowerCase().includes('ug/kg')||ad.toString().toLowerCase().includes('mg/kg')||ad.toString().toLowerCase().includes('mcg/kg')){
      const c=calcWeightDose(ad,W);if(c)h+=`<div class="dose"><span class="dl">⚡</span><span class="dv" style="color:var(--a);font-weight:700">${c} (${W}kg)</span></div>`;
    }
    if(pd.toString().toLowerCase().includes('/kg')||pd.toString().toLowerCase().includes('per kg')||pd.toString().toLowerCase().includes('ug/kg')||pd.toString().toLowerCase().includes('mg/kg')||pd.toString().toLowerCase().includes('mcg/kg')){
      const c=calcWeightDose(pd,W);if(c)h+=`<div class="dose"><span class="dl">⚡P</span><span class="dp dv">${c} (${W}kg)</span></div>`;
    }
  }
...
function showOnboarding(){
  const steps=[
    {t:'Search Anything',d:'Type a drug name, protocol, or score calculator in the search bar. Results appear instantly as you type.'},
    {t:'Set Patient Weight',d:'Enter the patient\'s weight in kg once. All weight-based doses and infusion calculations update automatically.'},
    {t:'Browse & Save',d:'Tap category tabs to browse. Tap the star to save favourites for quick access. Tap protocol cards to expand sections.'}
  ];
  let idx=0;
  ...
  function renderStep(){
    const s=steps[idx];
    let dots=steps.map((_,i)=>`<div class="obdot${i===idx?' on':''}"></div>`).join('');
    ob.innerHTML=`<div class="obc">
      <div class="obt">${s.t}</div>
      <div class="obd">${s.d}</div>
      <div class="obdots">${dots}</div>
      <div class="obbtns">
        <button class="obskip" onclick="closeOnboarding()">Skip</button>
        <button class="obn" onclick="nextStep()">${idx<steps.length-1?'Next':'Get Started'}</button>
      </div>
    </div>`;
  }
  window.nextStep=function(){idx++;if(idx>=steps.length){closeOnboarding();return}renderStep()};
  window.closeOnboarding=function(){localStorage.setItem('tr_onboarded','1');ob.remove();};
  renderStep();
