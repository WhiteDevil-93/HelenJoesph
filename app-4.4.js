...
/* ===== INFUSION DRUG DETECTOR ===== */
function isInfusionDrug(name, it){
  const n=(name||'').toLowerCase();
  const d=((it.adult_dose||it.adult_settings||it.paediatric_dose||it.paediatric_settings||'')+'').toLowerCase();
  const infusionNames=['noradrenaline','adrenaline','dopamine','dobutamine','phenylephrine','nitroglycerin','sodium nitroprusside','lignocaine','amiodarone','furosemide','morphine','midazolam','propofol','atracurium','alteplase','heparin','insulin'];
  if(infusionNames.some(x=>n.includes(x)))return true;
  if(d.includes('/min')||d.includes('infusion')||d.includes('ivi')||d.includes('mcg/kg/min')||d.includes('mg/kg/hr')||d.includes('units/hr'))return true;
  return false;
}
...
function calcInfusion(id){
  ...
  // mL/hr = (dose mcg/kg/min × wt kg × 60 min/hr) / (conc mg/mL × 1000 mcg/mg)
  const mlHr=(dose*wt*60)/(conc*1000);
  const dropsMin=mlHr*20/60; // assuming 20 gtt/mL
  resEl.style.display='block';
  resEl.innerHTML=`<div style="font-weight:700;color:var(--g);font-size:1rem">${mlHr.toFixed(1)} mL/hr</div>
    <div style="font-size:.68rem;color:var(--t2);margin-top:.1rem">Formula: (${dose} × ${wt}kg × 60) ÷ (${conc} × 1000) = ${mlHr.toFixed(2)} mL/hr</div>
